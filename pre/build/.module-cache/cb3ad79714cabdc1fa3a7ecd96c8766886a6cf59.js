...
// to URL path's.
initialState.week = Map({selected: null, count: 16, contestantStatus: null});
...
var week = function (prev, action) {
	...
	switch (action.type) {
		...
		return prev
			...
			.set('contestantStatus', action.payload.contestantStatus);
...
var contestants = function (prev, action) {
	...
	switch (action.type) {
		case 'SIGN-IN':
		return Map(action.payload.allContestants);
		default:
		return prev;
	}
};
