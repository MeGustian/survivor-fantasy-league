...
var initialState = I.fromJS({
	controller: {
		...
		error: {
			is: false
			,
			details: {action: undefined, messege: undefined}
		}
	}
	,
	navigation: {
	...
	profile: {
		newbie: true
		,
		chosen: [] // List of the id's of the chosen contestants.
	}
	,
	contestants: {
...
var profile = function (prev, action) {
	...
	switch (action.type) {
		case 'GET-INITIAL-DONE':
		return prev
			.set('chosen', I.fromJS(action.payload.chosen));
		case 'CHOOSE-CONTESTANTS-PEND':
		return prev
			.set('chosen', action.payload.chosen);
		case 'CHOOSE-CONTESTANTS-DONE':
		return prev;
		case 'CHOOSE-CONTESTANTS-FAIL':
		return prev
			.set('chosen', I.fromJS([]));
		default:
		return prev;
