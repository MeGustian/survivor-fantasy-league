	...
	switch (action.type) {
		...
		return prev
			.updateIn([
				...
				'togglingAchievements'
			], function (togglingAchievements) {
				if (typeof togglingAchievements === 'undefined') {
					return Set.of(action.payload.achievement);
				}
				return togglingAchievements.add(action.payload.achievement)
			})
			.updateIn([
				'contestantStatus',
				action.payload.contestantId,
		...
		return prev
			.updateIn([
				...
				return togglingAchievements.remove(action.payload.achievement)
			})
			.updateIn([
				'contestantStatus',
				action.payload.contestantId,
