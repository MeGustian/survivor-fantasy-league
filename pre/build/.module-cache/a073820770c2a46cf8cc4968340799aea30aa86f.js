	...
	render: function () {
		return (
			...
			React.createElement(Bs.Col, {sm: 12}, 
					React.createElement(Bs.Accordion, {bsStyle: "success", header: "Good achievements"}, 
						this.items('good')
					), 
					React.createElement(Bs.Accordion, {bsStyle: "danger", header: "Bad achievements"}, 
						this.items('bad')
					)
			)
			)
		...
		return React.addons.createFragment(
			that
				...
				.map(function (theAchievement, achievementCode) {
					return (
						React.createElement(Bs.Panel, {header: theAchievement.get('text'), key: achievementCode}, 
