...
var Fantasy = React.createClass({displayName: "Fantasy",
	render: function () {
		...
		var fullContestants = p.contestants.getIn(['statuses', p.navigation.get('selectedWeek').toString()]).mergeDeep(p.contestants.get('info'));
		var circumstances = {
	...
	propTypes: {
		...
		contestants: ImmutablePropTypes.contains({
			...
			statuses: ImmutablePropTypes.listOf(ImmutablePropTypes.mapOf(
				ImmutablePropTypes.contains({
