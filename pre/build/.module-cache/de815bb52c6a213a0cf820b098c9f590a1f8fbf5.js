...
var navigation = function (prev, action) {
	...
	switch (action.type) {
		...
		return prev
			...
			.update('selectedWeek', function (selected) {
				if (action.payload.target === 'week-last') {
					return (parseInt(prev.get('weekCount')) - 1).toString();
				}
				if (action.payload.target === 'week-upcoming') {
					return prev.get('weekCount');
				}
				return selected;
			});
		case 'WEEK-SELECT':
