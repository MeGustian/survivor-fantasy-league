...
var Fantasy = React.createClass({displayName: "Fantasy",
	render: function () {
		...
		console.log(Map.isMap(p.week.get('contestantStatus')));
		var fullContestants = p.week.get('contestantStatus').map(function (val) {
			console.log(val);
			return val.set('name', p.contestants.get('name'));
		});
		// var fullContestants = p.contestants.merge(p.week.get('contestantStatus'));
		console.log(p.week.get('contestantStatus').toJS());
		console.log(p.contestants.toJS());
		console.log(fullContestants.toJS());
