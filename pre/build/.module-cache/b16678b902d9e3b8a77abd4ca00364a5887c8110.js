		...
		return (
			React.createElement(Bs.OverlayTrigger, {placement: "top", overlay: tooltip, key: p.id, id: p.name}, 
				React.createElement(Bs.Thumbnail, {
					...
					alt: p.name, 
					style: {display: 'inline-block', border: (p.selected ? "3px solid green" : (p.emphasized ? "2px solid purple" : "")), width: '80px', marginBottom: '3px', marginRight: '10px', marginLeft: '10px', opacity: p.disabled ? '0.5' : ''}}
				)
			)
		);
	}
