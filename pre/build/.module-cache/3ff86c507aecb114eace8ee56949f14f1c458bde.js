...
var questions = function (prev, action) {
	...
	// if (action.payload.response && action.payload.response.body && typeof action.payload.response.body.error === 'object') {
	// 	var errorPayload = action.payload.response.body.error;
	// }
	var errorPayload = _.get(action.payload, 'response.body.error');
	switch (action.type) {
		...
		return prev
			.set(errorPayload.questionId, prev.getIn([
				errorPayload.questionId,
				'prev'
			]));
		// EDIT
		case 'EDIT-QUESTION':
